...
final class ConstructedString extends Structure implements StringType, Stringable
{
     ...
     */
    private static function decodeDefiniteLength(int $typeTag, string $data, int &$offset, int $length): self
    {
        $idx = $offset;
        $end = $idx + $length;
        $elements = [];
     ...
     * @param int $offset Offset to data
     */
    private static function decodeIndefiniteLength(int $typeTag, string $data, int &$offset): self
    {
        $idx = $offset;
        $elements = [];
