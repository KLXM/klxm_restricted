...
final class Uuid
{
    ...
    public static function uuid_create($uuidType = \UUID_TYPE_DEFAULT)
    {
        if (!is_numeric($uuidType) && null !== $uuidType) {
            trigger_error(sprintf('uuid_create() expects parameter 1 to be int, %s given', \gettype($uuidType)), \E_USER_WARNING);
        ...
        switch ((int) $uuidType) {
            ...
            default:
                trigger_error(sprintf("Unknown/invalid UUID type '%d' requested, using default type instead", $uuidType), \E_USER_WARNING);

                return self::uuid_generate_random();
        }
    }

    public static function uuid_generate_md5($uuidNs, $name)
    {
        if (!\is_string($uuidNs = self::toString($uuidNs))) {
            trigger_error(sprintf('uuid_generate_md5() expects parameter 1 to be string, %s given', \gettype($uuidNs)), \E_USER_WARNING);
        ...
        $hash = md5(hex2bin(str_replace('-', '', $uuidNs)).$name);
    ...
    public static function uuid_generate_sha1($uuidNs, $name)
    {
        if (!\is_string($uuidNs = self::toString($uuidNs))) {
            trigger_error(sprintf('uuid_generate_sha1() expects parameter 1 to be string, %s given', \gettype($uuidNs)), \E_USER_WARNING);
        ...
        $hash = sha1(hex2bin(str_replace('-', '', $uuidNs)).$name);
