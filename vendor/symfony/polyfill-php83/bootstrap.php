...
if (extension_loaded('mbstring')) {
    if (!function_exists('mb_str_pad')) {
        function mb_str_pad(string $string, int $length, string $padString = ' ', int $padType = STR_PAD_RIGHT, ?string $encoding = null): string { return p\Php83::mb_str_pad($string, $length, $padString, $padType, $encoding); }
    }
}
...
if (!function_exists('ldap_exop_sync') && function_exists('ldap_exop')) {
    function ldap_exop_sync($ldap, string $requestOid, ?string $requestData = null, ?array $controls = null, &$responseData = null, &$responseOid = null): bool { return ldap_exop($ldap, $requestOid, $requestData, $controls, $responseData, $responseOid); }
}

if (!function_exists('ldap_connect_wallet') && function_exists('ldap_connect')) {
    function ldap_connect_wallet(?string $uri, string $wallet, string $password, int $authMode = \GSLC_SSL_NO_AUTH) { return ldap_connect($uri, $wallet, $password, $authMode); }
}
