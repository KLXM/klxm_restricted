...
final class NegativeIntegerObject extends AbstractCBORObject implements Normalizable
{
    ...
    private static function createBigInteger(BigInteger $integer): self
    {
        ...
        $minusOne = BigInteger::of(-1);
        $computedValue = $minusOne->minus($integer);

        switch (true) {
            case $computedValue->isLessThan(BigInteger::of(24)):
                $ai = $computedValue->toInt();
                $data = null;
                break;
            case $computedValue->isLessThan(BigInteger::fromBase('FF', 16)):
                $ai = 24;
                $data = self::hex2bin(str_pad($computedValue->toBase(16), 2, '0', STR_PAD_LEFT));
                break;
            case $computedValue->isLessThan(BigInteger::fromBase('FFFF', 16)):
                $ai = 25;
                $data = self::hex2bin(str_pad($computedValue->toBase(16), 4, '0', STR_PAD_LEFT));
                break;
            case $computedValue->isLessThan(BigInteger::fromBase('FFFFFFFF', 16)):
                $ai = 26;
                $data = self::hex2bin(str_pad($computedValue->toBase(16), 8, '0', STR_PAD_LEFT));
                break;
            default:
