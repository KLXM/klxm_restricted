...
final class Real extends Element implements Stringable
{
     ...
     */
    private readonly BigInt $mantissa;

    /**
     * Signed integer exponent.
     */
    private readonly BigInt $exponent;
    ...
    private function __construct(BigInteger|int|string $mantissa, BigInteger|int|string $exponent, int $base = 10)
    {
        ...
        $this->strictDer = true;
        $this->mantissa = BigInt::create($mantissa);
        $this->exponent = BigInt::create($exponent);
        $this->base = $base;
    }
    ...
    {
        if (! isset($this->float)) {
            $m = $this->mantissa->toInt();
            $e = $this->exponent->toInt();
            $this->float = (float) ($m * $this->base ** $e);
        }
        return $this->float;
    }
    ...
    public function nr3Val(): string
    {
        ...
        if ($this->base === 2) {
            ...
            $m = $this->mantissa->getValue();
            $e = $this->exponent->getValue();
        }
    ...
    protected function encodedAsDER(): string
    {
        ...
        if ($this->exponent->getValue()->isEqualTo($infExponent)) {
        ...
        if ($this->mantissa->getValue()->toBase(10) === '0') {
            return '';
        }
        if ($this->base === 10) {
     ...
     */
    private function encodeBinary(): string
    {
        /** @var BigInteger $m */
        /** @var BigInteger $e */
        /** @var int $sign */
        ...
        $byte |= ($scale & 0x03) << 2;
        // encode exponent
        $expBytes = (BigInt::create($e))->signedOctets();
        $expLen = mb_strlen($expBytes, '8bit');
        if ($expLen > 0xff) {
     ...
     */
    private function encodeDecimal(): string
    {
     ...
     */
    private function encodeSpecial(): string
    {
        return match ($this->mantissa->toInt()) {
            1 => chr(0x40),
            -1 => chr(0x41),
     ...
     */
    private static function decodeBinaryEncoding(string $data): self
    {
        $byte = ord($data[0]);
     ...
     */
    private static function decodeDecimalEncoding(string $data): self
    {
        $nr = ord($data[0]) & 0x3f;
     ...
     */
    private static function decodeSpecialRealValue(string $data): self
    {
     ...
     */
    private function prepareBinaryEncoding(): array
    {
        $base = 2;
        $m = $this->mantissa->getValue();
        $ms = $m->getSign();
    ...
    private static function parsePHPExponentMatch(array $match): array
    {
        // mantissa sign
        $ms = $match['ms'] === '-' ? -1 : 1;
        $mParts = explode('.', $match['m']);
        // integer part of the mantissa
        $int = ltrim($mParts[0], '0');
        // exponent sign
        $es = $match['es'] === '-' ? -1 : 1;
        // signed exponent
