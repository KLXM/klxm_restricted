...
final class OpenSSLCrypto extends Crypto
{
    ...
    public function sign(
        ...
        $this->_checkSignatureAlgoAndKey($algo, $privkeyInfo->algorithmIdentifier());
        $result = openssl_sign($data, $signature, (string) $privkeyInfo->toPEM(), $this->_algoToDigest($algo));
        if ($result === false) {
    ...
    public function verify(
        ...
        $this->_checkSignatureAlgoAndKey($algo, $pubkeyInfo->algorithmIdentifier());
        $result = openssl_verify(
            $data,
            $signature->bitString()
                ->string(),
            (string) $pubkeyInfo->toPEM(),
     ...
     */
    private function _checkCipherKeySize(CipherAlgorithmIdentifier $algo, string $key): void
    {
     ...
     */
    private function _getLastError(): ?string
    {
     ...
     *
     * @param SignatureAlgorithmIdentifier $sigAlgo Signature algorithm
     * @param AlgorithmIdentifier $keyAlgo Key algorithm
     */
    private function _checkSignatureAlgoAndKey(
        SignatureAlgorithmIdentifier $sigAlgo,
        AlgorithmIdentifier $keyAlgo
    ): void {
        if (! $sigAlgo->supportsKeyAlgorithm($keyAlgo)) {
            throw new UnexpectedValueException(
                sprintf(
     ...
     */
    private function _algoToDigest(SignatureAlgorithmIdentifier $algo): int
    {
        $oid = $algo->oid();
     ...
     */
    private function _algoToCipher(CipherAlgorithmIdentifier $algo): string
    {
        $oid = $algo->oid();
     ...
     */
    private function _rc2AlgoToCipher(RC2CBCAlgorithmIdentifier $algo): string
    {
