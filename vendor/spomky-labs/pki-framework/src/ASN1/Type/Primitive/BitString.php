...
final class BitString extends BaseString
{
    ...
    public static function create(string $string, int $unusedBits = 0): self
    {
        return new self($string, $unusedBits);
    }
    ...
    public function withoutTrailingZeroes(): self
    {
        ...
        $unusedOctets = 0;
        for ($idx = mb_strlen($bits, '8bit') - 1; $idx >= 0; --$idx, ++$unusedOctets) {
            if ($bits[$idx] !== "\x0") {
                break;
            }
        }
        ...
        if ($unusedOctets !== 0) {
            $bits = mb_substr($bits, 0, -$unusedOctets, '8bit');
        }
        ...
        return self::create($bits, $unusedBits);
    }
    ...
    protected static function decodeFromDER(Identifier $identifier, string $data, int &$offset): ElementBase
    {
        ...
        $unusedBits = ord($data[$idx++]);
        if ($unusedBits > 7) {
            throw new DecodeException('Unused bits in a bit string must be less than 8.');
        }
        $strLen = $length->intLength() - 1;
        if ($strLen !== 0) {
            $str = mb_substr($data, $idx, $strLen, '8bit');
            if ($unusedBits !== 0) {
                $mask = (1 << $unusedBits) - 1;
        ...
        $offset = $idx + $strLen;
        return self::create($str, $unusedBits);
    }
}
