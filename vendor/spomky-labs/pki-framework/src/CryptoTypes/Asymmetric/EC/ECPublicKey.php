...
final class ECPublicKey extends PublicKey
{
    ...
    public static function fromCoordinates(
        ...
        if (! isset($bits) && isset($namedCurve)) {
            $bits = self::_curveSize($namedCurve);
        }
        $mlen = null;
        if (isset($bits)) {
            $mlen = (int) ceil($bits / 8);
        }
        $xOs = ECConversion::integerToOctetString(Integer::create($x), $mlen)->string();
        $yOs = ECConversion::integerToOctetString(Integer::create($y), $mlen)->string();
        $ecPoint = "\x4{$xOs}{$yOs}";
        return self::create($ecPoint, $namedCurve);
    }
