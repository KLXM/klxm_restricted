...
final class TBSCertificate
{
    ...
    public static function fromASN1(Sequence $seq): self
    {
        ...
        $tbsCert = self::create($subject, $pki, $issuer, $validity)
            ->withVersion($version)
            ->withSerialNumber($serial)
            ->withSignature($algo)
        ;
        if ($seq->hasTagged(1)) {
            $tbsCert = $tbsCert->withIssuerUniqueID(UniqueIdentifier::fromASN1(
                $seq->getTagged(1)
        ...
        if ($seq->hasTagged(2)) {
            $tbsCert = $tbsCert->withSubjectUniqueID(UniqueIdentifier::fromASN1(
                $seq->getTagged(2)
                    ->asImplicit(Element::TYPE_BIT_STRING)
                    ->asBitString()
            ));
        ...
        if ($seq->hasTagged(3)) {
            $tbsCert = $tbsCert->withExtensions(Extensions::fromASN1($seq->getTagged(3)->asExplicit()->asSequence()));
        }
        return $tbsCert;
    }
    ...
    public static function fromCSR(CertificationRequest $cr): self
    {
        ...
        if ($cri->hasAttributes()) {
            ...
            if ($attribs->hasExtensionRequest()) {
                $tbsCert = $tbsCert->withExtensions($attribs->extensionRequest()->extensions());
            }
        }
        // add Subject Key Identifier extension
        return $tbsCert->withAdditionalExtensions(
    ...
    public function withIssuerCertificate(Certificate $cert): self
    {
        ...
        $keyId = $cert->tbsCertificate()
            ...
            ->keyIdentifier();
        $obj->extensions = $obj->extensions->withExtensions(AuthorityKeyIdentifierExtension::create(false, $keyId));
        return $obj;
    }
     ...
     */
    public function withSubjectPublicKeyInfo(PublicKeyInfo $pubKeyInfo): self
    {
        $obj = clone $this;
        $obj->subjectPublicKeyInfo = $pubKeyInfo;
        return $obj;
    }
     ...
     * @param PrivateKeyInfo $privkeyInfo Private key used for signing
    ...
    public function sign(
        ...
        $tbsCert = clone $this;
        if (! isset($tbsCert->version)) {
            $tbsCert->version = $tbsCert->_determineVersion();
        }
        if (! isset($tbsCert->serialNumber)) {
            $tbsCert->serialNumber = '0';
        }
        $tbsCert->signature = $algo;
        $data = $tbsCert->toASN1()
            ->toDER();
        $signature = $crypto->sign($data, $privkeyInfo, $algo);
        return Certificate::create($tbsCert, $algo, $signature);
    }
