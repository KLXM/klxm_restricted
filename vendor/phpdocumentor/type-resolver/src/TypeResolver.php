...
final class TypeResolver
{
        ...
        switch (get_class($type)) {
            ...
            case ArrayShapeNode::class:
                return match ($type->kind) {
                    ArrayShapeNode::KIND_ARRAY => new ArrayShape(
                        ...array_map(
                    ...
                    ArrayShapeNode::KIND_LIST => new ListShape(
                        ...array_map(
                    ...
                    default => throw new RuntimeException('Unsupported array shape kind'),
                };
    ...
    {
        return match (true) {
            $type->constExpr instanceof ConstExprIntegerNode => new IntegerValue((int) $type->constExpr->value),
            $type->constExpr instanceof ConstExprFloatNode => new FloatValue((float) $type->constExpr->value),
            $type->constExpr instanceof ConstExprStringNode => new StringValue($type->constExpr->value),
            $type->constExpr instanceof ConstFetchNode => new ConstExpression(
                $this->resolve($type->constExpr->className, $context),
                $type->constExpr->name
            ),
            default => throw new RuntimeException(sprintf('Unsupported constant type %s', get_class($type))),
        };
    }
    ...
    {
        return match (true) {
            $this->isKeyword($type) => $this->resolveKeyword($type),
            $this->isFqsen($type) => $this->resolveTypedObject($type),
            $this->isPartialStructuralElementName($type) => $this->resolveTypedObject($type, $context),
