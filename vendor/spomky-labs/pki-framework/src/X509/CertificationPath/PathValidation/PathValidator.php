...
final class PathValidator
{
    ...
    {
        $pkInfo = $cert->tbsCertificate()
            ->subjectPublicKeyInfo();
        // assign working_public_key
        $state = $state->withWorkingPublicKey($pkInfo);
        // assign working_public_key_parameters
        $params = ValidatorState::getAlgorithmParameters($pkInfo->algorithmIdentifier());
        if ($params !== null) {
            $state = $state->withWorkingPublicKeyParameters($params);
        } else {
            // if algorithms differ, set parameters to null
            if ($pkInfo->algorithmIdentifier()->oid() !==
        ...
        return $state->withWorkingPublicKeyAlgorithm($pkInfo->algorithmIdentifier());
    }
    ...
    private function calculatePolicyIntersection(ValidatorState $state): ValidatorState
    {
        ...
        $initialPolicies = $this->config->policySet();
        if (in_array(PolicyInformation::OID_ANY_POLICY, $initialPolicies, true)) {
            return $state;
        }
        ...
        return $state->validPolicyTree()
            ->calculateIntersection($state, $initialPolicies);
    }
}
