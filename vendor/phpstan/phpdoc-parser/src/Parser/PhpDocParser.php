...
class PhpDocParser
{
	...
	public function parse(TokenIterator $tokens): PhpDocNode
	{
		...
		if (!$tokens->isCurrentTokenType(Lexer::TOKEN_CLOSE_PHPDOC)) {
			...
			while (!$tokens->isCurrentTokenType(Lexer::TOKEN_CLOSE_PHPDOC)) {
				if (
					...
					&& (
						$lastChild->value instanceof DoctrineTagValueNode
		...
		try {
			...
			$tag = new PhpDocTagNode(
				$name,
				$this->enrichWithAttributes(
					$tokens,
					new InvalidTagValueNode($e->getMessage(), $e),
					$startLine,
					$startIndex,
				),
			);
			...
			return $this->enrichWithAttributes($tokens, new PhpDocNode([$this->enrichWithAttributes($tokens, $tag, $startLine, $startIndex)]), 1, 0);
		}
	...
	/** @phpstan-impure */
	private function parseChild(TokenIterator $tokens): PhpDocChildNode
	{
		...
		if ($tokens->isCurrentTokenType(Lexer::TOKEN_DOCTRINE_TAG)) {
			...
			return $this->enrichWithAttributes($tokens, new PhpDocTagNode(
				$tag,
				$this->enrichWithAttributes(
					$tokens,
	 ...
	 * @return T
	 */
	private function enrichWithAttributes(TokenIterator $tokens, Node $tag, int $startLine, int $startIndex): Node
	{
		if ($this->config->useLinesAttributes) {
			$tag->setAttribute(Attribute::START_LINE, $startLine);
			$tag->setAttribute(Attribute::END_LINE, $tokens->currentTokenLine());
		}

		if ($this->config->useIndexAttributes) {
			$tag->setAttribute(Attribute::START_INDEX, $startIndex);
			$tag->setAttribute(Attribute::END_INDEX, $tokens->endIndexOfLastRelevantToken());
		}

		return $tag;
	}

	private function parseText(TokenIterator $tokens): PhpDocTextNode
	{
		$text = '';
		...
		return new PhpDocTextNode(trim($text, " \t"));
	}
			...
			if (!$tokens->isCurrentTokenType(Lexer::TOKEN_PHPDOC_EOL, Lexer::TOKEN_CLOSE_PHPDOC)) {
				...
				if ($tokens->isCurrentTokenType(Lexer::TOKEN_PHPDOC_TAG)) {
					...
					if ($child instanceof PhpDocTagNode) {
						if (
							$child->value instanceof GenericTagValueNode
	...
	public function parseTag(TokenIterator $tokens): PhpDocTagNode
	{
		$tag = $tokens->currentTokenValue();
		$tokens->next();
	...
	public function parseTagValue(TokenIterator $tokens, string $tag): PhpDocTagValueNode
	{
		$startLine = $tokens->currentTokenLine();
		$startIndex = $tokens->currentTokenIndex();
		...
		try {
			...
			switch ($tag) {
				...
				default:
					if ($tokens->isCurrentTokenType(Lexer::TOKEN_OPEN_PARENTHESES)) {
						...
						$tagValue = new GenericTagValueNode($this->parseOptionalDescriptionAfterDoctrineTag($tokens));
					}
					break;
			}
			...
			$tokens->rollback();
			$tagValue = new InvalidTagValueNode($this->parseOptionalDescription($tokens, false), $e);
		}

		return $this->enrichWithAttributes($tokens, $tagValue, $startLine, $startIndex);
	}

	private function parseDoctrineTagValue(TokenIterator $tokens, string $tag): PhpDocTagValueNode
	{
		$startLine = $tokens->currentTokenLine();
		$startIndex = $tokens->currentTokenIndex();

		return new DoctrineTagValueNode(
			$this->enrichWithAttributes(
				$tokens,
				new DoctrineAnnotation($tag, $this->parseDoctrineArguments($tokens, false)),
				$startLine,
				$startIndex,
			),
	...
	private function parseDoctrineArgument(TokenIterator $tokens): DoctrineArgument
	{
		if (!$tokens->isCurrentTokenType(Lexer::TOKEN_IDENTIFIER)) {
			...
			return $this->enrichWithAttributes(
				$tokens,
				new DoctrineArgument(null, $this->parseDoctrineArgumentValue($tokens)),
				$startLine,
				$startIndex,
			);
		}
		...
		try {
			...
			return $this->enrichWithAttributes(
				$tokens,
				new DoctrineArgument($key, $value),
				$startLine,
				$startIndex,
			);
	...
	private function parseDoctrineArgumentValue(TokenIterator $tokens)
	{
		...
		if ($tokens->isCurrentTokenType(Lexer::TOKEN_PHPDOC_TAG, Lexer::TOKEN_DOCTRINE_TAG)) {
			...
			return $this->enrichWithAttributes(
				$tokens,
				new DoctrineAnnotation($name, $this->parseDoctrineArguments($tokens, true)),
				$startLine,
				$startIndex,
			);
		}
	...
	private function parseDoctrineArrayItem(TokenIterator $tokens): DoctrineArrayItem
	{
		$startLine = $tokens->currentTokenLine();
		$startIndex = $tokens->currentTokenIndex();
		...
		try {
			...
			return $this->enrichWithAttributes(
				$tokens,
				new DoctrineArrayItem($key, $value),
				$startLine,
				$startIndex,
			);
		} catch (ParserException) {
			...
			return $this->enrichWithAttributes(
				$tokens,
				new DoctrineArrayItem(null, $this->parseDoctrineArgumentValue($tokens)),
				$startLine,
				$startIndex,
			);
		}
	...
	/**
     * @return ParamTagValueNode|TypelessParamTagValueNode
     */
    private function parseParamTagValue(TokenIterator $tokens): PhpDocTagValueNode
	{
		if (
		...
		if ($type !== null) {
			return new ParamTagValueNode($type, $isVariadic, $parameterName, $description, $isReference);
		}

		return new TypelessParamTagValueNode($isVariadic, $parameterName, $description, $isReference);
	}

	private function parseParamImmediatelyInvokedCallableTagValue(TokenIterator $tokens): ParamImmediatelyInvokedCallableTagValueNode
	{
		$parameterName = $this->parseRequiredVariableName($tokens);
		$description = $this->parseOptionalDescription($tokens, false);

		return new ParamImmediatelyInvokedCallableTagValueNode($parameterName, $description);
	}

	private function parseParamLaterInvokedCallableTagValue(TokenIterator $tokens): ParamLaterInvokedCallableTagValueNode
	{
		$parameterName = $this->parseRequiredVariableName($tokens);
		$description = $this->parseOptionalDescription($tokens, false);

		return new ParamLaterInvokedCallableTagValueNode($parameterName, $description);
	}

	private function parseParamClosureThisTagValue(TokenIterator $tokens): ParamClosureThisTagValueNode
	{
		$type = $this->typeParser->parse($tokens);
		$parameterName = $this->parseRequiredVariableName($tokens);
		$description = $this->parseOptionalDescription($tokens, false);
	...
	private function parsePureUnlessCallableIsImpureTagValue(TokenIterator $tokens): PureUnlessCallableIsImpureTagValueNode
	{
		$parameterName = $this->parseRequiredVariableName($tokens);
		$description = $this->parseOptionalDescription($tokens, false);

		return new PureUnlessCallableIsImpureTagValueNode($parameterName, $description);
	}

	private function parseVarTagValue(TokenIterator $tokens): VarTagValueNode
	{
		$type = $this->typeParser->parse($tokens);
		$variableName = $this->parseOptionalVariableName($tokens);
		$description = $this->parseOptionalDescription($tokens, $variableName === '');
		return new VarTagValueNode($type, $variableName, $description);
	}

	private function parseReturnTagValue(TokenIterator $tokens): ReturnTagValueNode
	{
		$type = $this->typeParser->parse($tokens);
	...
	private function parseThrowsTagValue(TokenIterator $tokens): ThrowsTagValueNode
	{
		$type = $this->typeParser->parse($tokens);
		$description = $this->parseOptionalDescription($tokens, true);
	...
	private function parseMixinTagValue(TokenIterator $tokens): MixinTagValueNode
	{
		$type = $this->typeParser->parse($tokens);
		$description = $this->parseOptionalDescription($tokens, true);
	...
	private function parseRequireExtendsTagValue(TokenIterator $tokens): RequireExtendsTagValueNode
	{
		$type = $this->typeParser->parse($tokens);
		$description = $this->parseOptionalDescription($tokens, true);
	...
	private function parseRequireImplementsTagValue(TokenIterator $tokens): RequireImplementsTagValueNode
	{
		$type = $this->typeParser->parse($tokens);
		$description = $this->parseOptionalDescription($tokens, true);
	...
	private function parseSealedTagValue(TokenIterator $tokens): SealedTagValueNode
	{
		$type = $this->typeParser->parse($tokens);
		$description = $this->parseOptionalDescription($tokens, true);
	...
	private function parseDeprecatedTagValue(TokenIterator $tokens): DeprecatedTagValueNode
	{
		$description = $this->parseOptionalDescription($tokens, false);
		return new DeprecatedTagValueNode($description);
	}

	private function parsePropertyTagValue(TokenIterator $tokens): PropertyTagValueNode
	{
		$type = $this->typeParser->parse($tokens);
		$parameterName = $this->parseRequiredVariableName($tokens);
		$description = $this->parseOptionalDescription($tokens, false);
		return new PropertyTagValueNode($type, $parameterName, $description);
	}

	private function parseMethodTagValue(TokenIterator $tokens): MethodTagValueNode
	{
		...
		$description = $this->parseOptionalDescription($tokens, false);
		return new MethodTagValueNode($isStatic, $returnType, $methodName, $parameters, $description, $templateTypes);
	}

	private function parseMethodTagValueParameter(TokenIterator $tokens): MethodTagValueParameterNode
	{
		$startLine = $tokens->currentTokenLine();
		$startIndex = $tokens->currentTokenIndex();
		...
		return $this->enrichWithAttributes(
			$tokens,
			new MethodTagValueParameterNode($parameterType, $isReference, $isVariadic, $parameterName, $defaultValue),
			$startLine,
			$startIndex,
		);
	}

	private function parseExtendsTagValue(string $tagName, TokenIterator $tokens): PhpDocTagValueNode
	{
		$startLine = $tokens->currentTokenLine();
		$startIndex = $tokens->currentTokenIndex();
        ...
        return match ($tagName) {
            '@extends' => new ExtendsTagValueNode($type, $description),
            '@implements' => new ImplementsTagValueNode($type, $description),
            '@use' => new UsesTagValueNode($type, $description),
            default => throw new ShouldNotHappenException(),
        };
	}

	private function parseTypeAliasTagValue(TokenIterator $tokens): TypeAliasTagValueNode
	{
		...
		try {
			...
			return new TypeAliasTagValueNode(
				$alias,
				$this->enrichWithAttributes($tokens, new InvalidTypeNode($e), $startLine, $startIndex),
			);
		}
	}

	private function parseTypeAliasImportTagValue(TokenIterator $tokens): TypeAliasImportTagValueNode
	{
		...
		return new TypeAliasImportTagValueNode($importedAlias, $importedFromType, $importedAs);
	}

	/**
     * @return AssertTagValueNode|AssertTagPropertyValueNode|AssertTagMethodValueNode
     */
    private function parseAssertTagValue(TokenIterator $tokens): PhpDocTagValueNode
	{
		...
		if (array_key_exists('method', $parameter)) {
			return new AssertTagMethodValueNode($type, $parameter['parameter'], $parameter['method'], $isNegated, $description, $isEquality);
		} elseif (array_key_exists('property', $parameter)) {
			return new AssertTagPropertyValueNode($type, $parameter['parameter'], $parameter['property'], $isNegated, $description, $isEquality);
		}

		return new AssertTagValueNode($type, $parameter['parameter'], $isNegated, $description, $isEquality);
	}
	...
	private function parseSelfOutTagValue(TokenIterator $tokens): SelfOutTagValueNode
	{
		$type = $this->typeParser->parse($tokens);
		$description = $this->parseOptionalDescription($tokens, true);
	...
	private function parseParamOutTagValue(TokenIterator $tokens): ParamOutTagValueNode
	{
		$type = $this->typeParser->parse($tokens);
		$parameterName = $this->parseRequiredVariableName($tokens);
		$description = $this->parseOptionalDescription($tokens, false);

		return new ParamOutTagValueNode($type, $parameterName, $description);
	}
