...
final class PolicyTree
{
    ...
    {
        $tree = clone $this;
        $validPolicyNodeSet = $tree->validPolicyNodeSet();
        ...
        $validPolicyNodeSet = array_filter(
            $validPolicyNodeSet,
        ...
        $validPolicySet = array_map(static fn (PolicyNode $node) => $node->validPolicy(), $validPolicyNodeSet);
        ...
        foreach ($tree->nodesAtDepth($state->index()) as $node) {
            if ($node->hasParent() && $node->isAnyPolicy()) {
                ...
                $poids = array_diff($policies, $validPolicySet);
    ...
    private function processPolicy(PolicyInformation $policy, ValidatorState $state): void
    {
        ...
        foreach ($this->nodesAtDepth($i - 1) as $node) {
            ...
            if ($node->hasExpectedPolicy($pOid)) {
                $node->addChild(PolicyNode::create($pOid, $policy->qualifiers(), [$pOid]));
                ++$matchCount;
            }
        }
    ...
    private function processAnyPolicy(PolicyInformation $policy, Certificate $cert, ValidatorState $state): void
    {
        ...
        foreach ($this->nodesAtDepth($i - 1) as $node) {
            ...
            foreach ($node->expectedPolicies() as $pOid) {
                // that does not appear in a child node
                if (! $node->hasChildWithValidPolicy($pOid)) {
                    $node->addChild(PolicyNode::create($pOid, $policy->qualifiers(), [$pOid]));
                }
            }
        }
    ...
    {
        $policyMappings = $cert->tbsCertificate()
            ->extensions()
            ->policyMappings();
        // (6.1.4. b.1.) for each node in the valid_policy_tree of depth i...
        foreach ($policyMappings->flattenedMappings() as $idp => $sdps) {
            $matchCount = 0;
