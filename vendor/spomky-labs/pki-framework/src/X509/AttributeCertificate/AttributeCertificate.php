...
final class AttributeCertificate implements Stringable
{
    ...
    {
        return $this->acInfo->holder()->identifiesPKC($cert);
    }
    ...
    {
        return $this->acInfo->issuer()->identifiesPKC($cert);
    }

    /**
     * Verify signature.
     *
     * @param PublicKeyInfo $pubkeyInfo Signer's public key
     * @param null|Crypto $crypto Crypto engine, use default if not set
     */
    public function verify(PublicKeyInfo $pubkeyInfo, ?Crypto $crypto = null): bool
    {
        $crypto ??= Crypto::getDefault();
        $data = $this->acInfo->toASN1()
            ->toDER();
        return $crypto->verify($data, $this->signatureValue, $pubkeyInfo, $this->signatureAlgorithm);
    }
}
