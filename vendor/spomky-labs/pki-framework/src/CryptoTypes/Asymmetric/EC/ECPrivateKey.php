...
final class ECPrivateKey extends PrivateKey
{
    ...
    public static function fromASN1(Sequence $seq): self
    {
        ...
        if ($seq->hasTagged(0)) {
            $params = $seq->getTagged(0)
                ->asExplicit();
            $namedCurve = $params->asObjectIdentifier()
                ->oid();
        }
        $publicKey = null;
        if ($seq->hasTagged(1)) {
            $publicKey = $seq->getTagged(1)
                ->asExplicit()
                ->asBitString()
                ->string();
        }
        return self::create($privateKey, $namedCurve, $publicKey);
    }
     ...
     */
    public function withNamedCurve(?string $namedCurve): self
    {
        $obj = clone $this;
