...
final class RDN implements Countable, IteratorAggregate, Stringable
{
     ...
     */
    private readonly array $attribs;
    ...
    {
        $elements = array_map(static fn (AttributeTypeAndValue $tv) => $tv->toASN1(), $this->attribs);
    ...
    {
        $parts = array_map(static fn (AttributeTypeAndValue $tv) => $tv->toString(), $this->attribs);
    ...
    public function equals(self $other): bool
    {
        ...
        $attribs1 = $this->attribs;
        $attribs2 = $other->attribs;
        // if there's multiple attributes, sort using SET OF rules
        if (count($attribs1) > 1) {
            $attribs1 = self::fromASN1($this->toASN1())->attribs;
            $attribs2 = self::fromASN1($other->toASN1())->attribs;
        }
    ...
    public function allOf(string $name): array
    {
        ...
        $attribs = array_filter($this->attribs, static fn (AttributeTypeAndValue $tv) => $tv->oid() === $oid);
    ...
    {
        return new ArrayIterator($this->attribs);
    }
}
