...
final class RC2CBCAlgorithmIdentifier extends BlockCipherAlgorithmIdentifier
{
    ...
    public static function create(int $effectiveKeyBits = 64, ?string $iv = null): self
    {
        return new self($effectiveKeyBits, $iv);
    }
        ...
        if ($params->isType(Element::TYPE_OCTET_STRING)) {
            ...
            if ($seq->has($idx, Element::TYPE_INTEGER)) {
                $version = $seq->at($idx++)
                    ->asInteger()
                    ->intNumber();
                $keyBits = self::_versionToEKB($version);
            }
