...
final class Holder
{
    ...
    public static function fromASN1(Sequence $seq): self
    {
        ...
        if ($seq->hasTagged(1)) {
            $entityName = GeneralNames::fromASN1(
                $seq->getTagged(1)
                    ->asImplicit(Element::TYPE_SEQUENCE)
                    ->asSequence()
            );
        ...
        if ($seq->hasTagged(2)) {
            $digestInfo = ObjectDigestInfo::fromASN1(
                $seq->getTagged(2)
                    ->asImplicit(Element::TYPE_SEQUENCE)
                    ->asSequence()
            );
        }
        return self::create($certId, $entityName)
            ->withObjectDigestInfo($digestInfo);
    }
